...
public class GenerateBinaryMojo extends GenerateMojo {
    ...
    protected void executeImpl(ModuleInfo target) throws Exception {
        ...
        for (ModuleInfo.Annotation annotation : target.getAnnotations()) {
            AnnotationVisitor visited = classWriter.visitAnnotation("L" + className(annotation.getClassName()) + ";", true);
            visited.visitEnd();
        }
